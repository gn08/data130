# Interactive Greeting Project
## About This Project
This is my first project stored in my Data Science GitHub portfolio.
## What It Does
This Python program asks the user for their name and displays a personalized greeting.
## Tools Used
- Python
- GitHub
- Markdown
## What I Learned
Today I learned the basics of using github and how to store files in an organized matter. This is really helpful because I was not taught to do this for other classes so I have files just floating around with just the class name in the file name, but they are not organized within folders. Something especially helpful that stood out to me is that when naming the file if you want it to be a folder you can include a "/" after the name. Another tool that should be utilized in github is always creating a "README.md" file primarily when creating your projects as this creates an introduction and maintains the clearness in your project.

## Part 2 - Think About the Code
- What do you think input() does?
    The input function takes in an input that the user can enter and assigns it to a variable.
- What do you think print() does?
    The print function prints out a statement to the user.
- In your own words, what does this program do?
    The file is assigned to function greet_user(). The program first prompts the user to enter their name and then assigns that to the name variable. It then prints out a introductory welcome using the user's name that was entered. At the end(the last two lines) the function greet_user() is ran the program prompts to run the input and print statement.
    
